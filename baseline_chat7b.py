...
suffix = ' What is the answer to this question?'
...
for sentences, answers in data_loader:
    dialogs: List[Dialog] = [[{"role": "user", "content":  sentence + suffix}] for sentence in sentences]
    ...
    for prompt, result, answer in zip(dialogs, results, answers):
        pattern = r'\([A-Za-z]\)'
        match = re.search(pattern, result['generation']['content'])
        if match is None:
            continue
        model_answer = match.group(0)
        if model_answer is not None and model_answer[1] == answer:
            acc += 1
        # print(prompt),
