...
class LMGNN(Transformer):
    ...
    def forward(self, tokens, token_to_node_index, start_pos, graphs):
        ...
        # h = torch.concat((h, entity_emb[:self.params.max_seq_len - seqlen, :].unsqueeze(0).half()), dim=1)
        # seqlen += entity_emb.shape[0]
        self.freqs_cis = self.freqs_cis.to(h.device)
        freqs_cis = self.freqs_cis[start_pos : start_pos + seqlen]
        ...
        # full_mask[:,:min_len,:] = info_mask
        # h = torch.where(full_mask, h_new + entity_to_token.half(), h_new)

        # separate the tensor to two tensors
        # sep = h[:,:min_len,:][info_mask]
        # # sep = sep + entity_to_token[:,:min_len,:][info_mask].half()
        # sep = sep.reshape(h.shape[0], -1, h.shape[2])
        # h = torch.concat((sep, h[:,min_len:,:]), dim=1)
