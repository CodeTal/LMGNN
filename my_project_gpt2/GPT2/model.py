...
class GAT(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, heads, num_layers):
        ...
        for _ in range(num_layers - 2):
            self.convs.append(GATConv(hidden_dim * heads, hidden_dim, heads))
        self.convs.append(GATConv(hidden_dim * heads, out_dim, heads, concat=False))
...
class LMGNNGPT2Model(nn.Module):
    def __init__(self, config):
        super(LMGNNGPT2Model, self).__init__()
        self.n_layer = config.n_layer
        self.n_embd = config.n_embd
        self.n_vocab = config.vocab_size
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, past=None, entity_emb=None):
        if past is None:
            past_length = 0
            past = [None] * len(self.h)
        else:
        ...
        if token_type_ids is not None:
            ...
            token_type_embeds = 0
        hidden_states = inputs_embeds + position_embeds + token_type_embeds + entity_emb
        presents = []
        for block, layer_past in zip(self.h, past):
            hidden_states, present = block(hidden_states, layer_past)
            presents.append(present)
...
class LMGNN(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = LMGNNGPT2Model(config)
        self.lm_head = GPT2LMHead(self.transformer.wte.weight, config)
        self.gnn = GAT(1, 1000, config.n_embd, 3, 3)
    ...
    def forward(
        ...
        hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past, entity_to_token)
        lm_logits = self.lm_head(hidden_states)
        if lm_labels is not None:
            loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
            loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), lm_labels.view(-1))
